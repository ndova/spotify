import os
import sys
from itunes_client import iTunesClient
from downloader import AudioDownloader

file_path = os.environ.get('TARGET_FILE')
search_title = os.environ.get('SEARCH_TITLE')

if not file_path:
    print('Please set TARGET_FILE environment variable')
    sys.exit(1)

file_path = os.path.abspath(file_path)
if not os.path.exists(file_path):
    print('File not found:', file_path)
    sys.exit(1)

if not search_title:
    search_title = os.path.splitext(os.path.basename(file_path))[0]

print('Target file:', file_path)
print('Search title:', search_title)

cli = iTunesClient()
results = cli.search_tracks(search_title, limit=5)
print('Found', len(results), 'results')
if results:
    first = results[0]
    print('Using:', first.get('artist'), '-', first.get('title'), '| album:', first.get('album'))
    ad = AudioDownloader(os.path.dirname(file_path))
    ad._add_metadata(file_path, first)
    print('Metadata write attempted')
else:
    print('No metadata found for query')
